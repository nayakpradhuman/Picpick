...
import {
  Check, MessageSquare, Users, Images, Star, ArrowLeft,
  Share2, ExternalLink, ChevronDown, RefreshCw, Trash2, Download,
} from "lucide-react";
import { Button } from "../components/ui/button";
import {
  getAlbum, getReviewsForAlbum, deleteAlbum, revokePhotoUrls,
  type Album, type Review,
} from "../lib/storage";
...
export function Results() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (found) {
        setAlbum(found);
        setReviews(await getReviewsForAlbum(id));
      } else {
        setNotFound(true);
      }
      setLoading(false);
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      if (id) setReviews(await getReviewsForAlbum(id));
    }, 3000);
  ...
  const handleExport = () => {
    ...
    if (reviews.length > 0) {
      ...
      sortedPhotos.forEach((photo, i) => {
        ...
        if (suggestions.length > 0) text += `, ${suggestions.length} suggestion${suggestions.length !== 1 ? "s" : ""}`;
        text += "\n";
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#fdf0f8] via-white to-[#f0f0ff] flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <Button onClick={() => navigate("/")} className="bg-gradient-to-r from-[#f472b6] to-[#8b5cf6] text-white border-0 h-11 rounded-xl">
            Go Home
          </Button>
        </div>
      </div>
  ...
  const totalSuggestions = reviews.reduce((acc, r) => acc + r.feedback.filter((f) => f.text).length, 0);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#fdf0f8] via-white to-[#f0f0ff]">
      ...
      <header className="border-b border-black/5 bg-white/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-3xl mx-auto px-4 py-3.5 flex items-center gap-2.5">
          <button className="w-9 h-9 flex items-center justify-center rounded-xl active:bg-black/5 transition-colors shrink-0" onClick={() => navigate("/")}>
          ...
          <div className="flex-1 min-w-0">
            <span className="text-[#1a1a2e] truncate block text-sm" style={{ fontWeight: 600 }}>{album.title}</span>
          </div>
          <Link to={`/album/${id}`} className="flex items-center gap-1.5 text-sm text-[#8b5cf6] active:text-[#f472b6] transition-colors shrink-0 py-1 px-2 rounded-lg">
      ...
      <main className="max-w-3xl mx-auto px-4 py-5 pb-12">
        {/* Share card */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-black/5 p-4 mb-4">
          <div className="flex items-center gap-2 mb-3">
            <Share2 className="w-4 h-4 text-[#8b5cf6] shrink-0" />
            <span className="text-[#1a1a2e] text-sm" style={{ fontWeight: 500 }}>Share with friends</span>
          </div>
          ...
          <Button
            className={`w-full h-11 rounded-xl border-0 transition-all ${copied ? "bg-green-500 text-white" : "bg-gradient-to-r from-[#f472b6] to-[#8b5cf6] text-white"}`}
            onClick={handleShare}
          >
            {copied ? <><Check className="w-4 h-4 mr-2" /> Copied!</> : <><Share2 className="w-4 h-4 mr-2" /> Share Link</>}
          </Button>
          <p className="text-xs text-[#717182] mt-2.5 leading-relaxed">
            ⚠️ Data is stored in this browser's IndexedDB. The review link works only on the same device/browser.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-3 gap-2.5 mb-4">
          {[
            ...
            <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl border border-black/5 p-3 text-center">
          ...
          <div className="px-4 py-3.5 border-b border-black/5 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[#1a1a2e] text-sm" style={{ fontWeight: 500 }}>
                Photo Results{reviews.length === 0 && <span className="text-[#717182]"> — waiting for responses</span>}
              </span>
            </div>
            <button className="w-7 h-7 flex items-center justify-center rounded-lg active:bg-black/5 transition-colors" onClick={manualRefresh} title="Refresh">
                ...
                {sortedPhotos.map((photo, i) => {
                  ...
                  return (
                    <motion.div key={photo.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.04 }}>
                      <div
                        className={`relative aspect-square rounded-2xl overflow-hidden cursor-pointer active:scale-[0.97] transition-transform ${isTop ? "ring-[3px] ring-[#f472b6] ring-offset-2" : ""}`}
                      ...
                      >
                        ...
                        <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/65 to-transparent p-2.5 flex items-center gap-2">
                          <div className="flex items-center gap-1 text-white text-xs"><Check className="w-3 h-3" /><span>{votes}</span></div>
                          {suggestions.length > 0 && <div className="flex items-center gap-1 text-white text-xs"><MessageSquare className="w-3 h-3" /><span>{suggestions.length}</span></div>}
                          {(votes > 0 || suggestions.length > 0) && <ChevronDown className={`w-3.5 h-3.5 text-white ml-auto transition-transform ${isExpanded ? "rotate-180" : ""}`} />}
                        </div>
                      </div>
                      <div className="mt-1.5 h-1.5 rounded-full bg-gray-100 overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-[#f472b6] to-[#8b5cf6] rounded-full transition-all duration-700" style={{ width: maxVotes > 0 ? `${(votes / maxVotes) * 100}%` : "0%" }} />
                      </div>
                      <AnimatePresence>
                        {isExpanded && (
                                ...
                                <div className="space-y-2">
                                  ...
                                  {suggestions.map((s, si) => (
                                    <div key={si} className="text-xs text-[#1a1a2e] leading-relaxed">
                                      <span className="text-[#8b5cf6]" style={{ fontWeight: 600 }}>{s.reviewer}: </span>{s.text}
                                    </div>
                                  ))}
                                </div>
                              ) : (
        ...
        {reviews.length > 0 && (
          <div className="bg-white rounded-2xl border border-black/5 overflow-hidden mb-4">
            <div className="px-4 py-3.5 border-b border-black/5 flex items-center gap-2">
              ...
              <span className="text-[#1a1a2e] text-sm" style={{ fontWeight: 500 }}>Individual Responses ({reviews.length})</span>
            </div>
            <div className="divide-y divide-black/5">
              {reviews.map((review) => {
                ...
                return (
                  <div key={review.id}>
                    <button className="w-full px-4 py-3.5 flex items-center gap-3 active:bg-gray-50 transition-colors text-left" onClick={() => setExpandedReviewId(isOpen ? null : review.id)}>
                      <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#f472b6] to-[#8b5cf6] flex items-center justify-center text-white text-xs shrink-0" style={{ fontWeight: 700 }}>
                        {review.reviewerName.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[#1a1a2e] text-sm truncate" style={{ fontWeight: 500 }}>{review.reviewerName}</p>
                      ...
                      <ChevronDown className={`w-4 h-4 text-[#717182] transition-transform shrink-0 ${isOpen ? "rotate-180" : ""}`} />
                    </button>
                    <AnimatePresence>
                      {isOpen && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                            ...
                            {review.feedback.filter((f) => f.text).map((f, fi) => {
                              ...
                              return (
                                <div key={fi} className="flex items-start gap-2.5 bg-[#fdf0f8] rounded-xl p-3">
                                  {p && <div className="w-9 h-9 rounded-lg shrink-0 overflow-hidden"><img src={p.url} alt="" className="w-full h-full object-cover" /></div>}
        ...
        <div className="flex gap-2.5">
          {reviews.length > 0 && (
            <Button className="flex-1 h-11 rounded-xl bg-white border border-black/10 text-[#1a1a2e] hover:bg-gray-50 active:bg-gray-100" onClick={handleExport}>
              <Download className="w-4 h-4 mr-2 text-[#8b5cf6]" /> Export Results
            </Button>
          )}
          <Button className="flex-1 h-11 rounded-xl bg-white border border-red-200 text-red-500 hover:bg-red-50 active:bg-red-100" onClick={handleDelete}>
